	...
	public function generate_callback() {
		if($this->input->post()){
			...
			if($this->input->post('ref_by'))
			{
				...
				'leadid'=> trim("OLV-".sprintf("%'.08d",$lead_ids).PHP_EOL),
				'user_id'=>$user_name,
				'due_date'=>$due_date,
...
{		
			...
			if (move_uploaded_file($_FILES["upload_xl"]["tmp_name"], $target)){
				...
				for($i = 2;$i <= $highestRow; $i++ ){
					...
					$data = array(
						...
						'email1'=>trim($email1),
						'email2'=>trim($email2),
						'leadid'=>trim("OLV-".sprintf("%'.011d",$lead_ids++).PHP_EOL),
						'notes'=>trim($notes),
					);
					// print_r($data);exit;
	...
	public function save_online_leads(){
		...
		if($this->input->post()){
			...
			if($this->input->post('delete_leads'))
		    {
		        if(count($checked)>1)
		        {
		        foreach ($checked as $key) {    
        		$lead_id = array('id'=>$key);
        		$bool = $this->common_model->updateWhere_leadid($key);
		        } 
		        }
		    } 
			...
			// 	}
				//echo site_url()
				if($this->input->post('delete_leads'))
				echo "<script>alert('deleted successfully');location.href='".base_url().'admin/'.$ext."'</script>";
				elseif($error==0)
